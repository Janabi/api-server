...
describe('Checking API Routes for both Products and Categories', ()=>{
    ...
    it('can put() a category item', async()=> {
        ...
        const data = await mockRequest.post('/categories').send(categoryObj);
        const record = data.body;
        const categoryItemResponse =  await mockRequest.put(`/categories/${record._id}`).send(categoryObjUpdate);
        const categoryItem = categoryItemResponse.body[0];
    ...
    it('can delete() a category item', async()=> {
        ...
        const data = await mockRequest.post('/categories').send(categoryObj);
        const record = data.body;
        const categoryItemResponse =  await mockRequest.delete(`/categories/${record._id}`);
    ...
    it('can put() a product item', async()=> {
        ...
        const data = await mockRequest.post('/products').send(productObj);
        const record = data.body;
        const productItemResponse =  await mockRequest.put(`/products/${record._id}`).send(productObjUpdate);
        const productItem = productItemResponse.body[0];
    ...
    it('can delete() a product item', async()=> {
        ...
        const data = await mockRequest.post('/products').send(productObj);
        const record = data.body;
        const productItemResponse =  await mockRequest.delete(`/products/${record._id}`);
